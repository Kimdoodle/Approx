...
def remez_algorithm(n: int, intervals: list, evalF, approx_mode: str, eb: EB, err_mode: int, print_mode: str, pp: list=[]) -> tuple:  
    try:
        # 1: x 샘플링
        # 2: 행렬식 구성
        match approx_mode:
            ...
            case "cl":
                sample_number = pp.count(1) + 1
                powers = [0, n-1, n]
            case "etc":
                sample_number = pp.count(1) + 1
                powers = [i for i, v in enumerate(pp) if v == 1]
                
        x_samples = sample_points_multi(sample_number, intervals, print_mode)
            
    except Exception as e:
        print("remez algorithm init error")
        print(e)
        return [-1], 99, 99, []
    ...
    try:
        for _ in range(50):
            ...
            # 3: 행렬식 연산
            assert A_matrix.shape[0] == A_matrix.shape[1], f"A가 직사각 행렬입니다. shape={A_matrix.shape[0]}, {A_matrix.shape[1]}"
            coeff, E = solve_matrix(A_matrix, y_matrix, n, powers)

            if all(i==0.0 for i in coeff):
                return [-1], 99, 99, []
            # 4: 지역 극값 연산
            max_point_x, max_point_y = calculate_local_max(coeff, evalF, intervals)

            if len(max_point_x) == sample_number:
                best_points_x, best_points_y = max_point_x, max_point_y
            ...
            if decide_exit(best_error_abs, 1e-2, print_mode):
                try:
                    ...
                    if next_intervals[0][1] < next_intervals[1][0]:
                        return coeff, max_err0, max_err1, next_intervals
                    else:
        ...
        print("remez algorithm iteration error")
        print(e)
        return [-1], 99, 99, []


def remez_algorithm_init(n: int, intervals: list, evalF, eb: EB, err_mode: int, print_mode: str, pp: list, coeff: list) -> tuple:  
    # intervals = slice_interval(approx_mode, intervals)
